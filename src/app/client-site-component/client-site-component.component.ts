...
export class ClientSiteComponentComponent {
  ...
  columnsToDisplay3 = ['modelOfDevice', 'sim' ,'simPhone',  'deviceNumPostfix', 'fiscalNumPostfix', 'napNumber', 'napDate', 'Actions'];
  ...
  columnHeadersDevices = ['Модел', 'СИМ IMSI', 'СИМ номер', 'Сериен номер', 'Фискална памет', 'НАП номер', 'НАП дата', 'Действия'];
  ...
  generateRequest(elementId: any){
    this.documentGeneratorService.generateRequest(elementId).subscribe(docResult => {
